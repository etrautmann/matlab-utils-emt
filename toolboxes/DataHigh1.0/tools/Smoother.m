...
%
%   Drawbacks:  Ideally, we would use the original data each time, smooth
%   it to your heart's content, upload it to DataHigh, and then come back
%   to smooth the original data.  However, the original data can have a
%   larger dimensionality than 17, so I'd first have to perform PCA on it,
%   changing the bases (and thus changing the projection).  I also don't
%   want to save an entire D struct just to keep it for smoothing.  So
%   any changes that the user makes will be saved on D, and can't be undone
%   unless you reload the data.  Future work will correct this.
%
...
% Opening function
    ...
    handles.replace = 0;  %do not replace with smooth data
...
% Upload the current data to DataHigh
    ...
    handles.replace = ~handles.replace;
    guidata(hObject, handles);
