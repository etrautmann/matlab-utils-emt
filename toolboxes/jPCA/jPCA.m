...
% 
% 
function [Projection, Summary, pcaData] = jPCA(Data, analyzeTimes, params)
...
function plotRosette(Proj, whichPair)
    ...
    arrowSize = 5;
%     colors = jet(numConds);
    colors = flipud(cbrewer('div','RdYlBu',numConds,'cubic'));
    for c = 1:numConds
%         plot(Proj(c).proj(:,d1), Proj(c).proj(:,d2), 'k');
%         plot(Proj(c).proj(1,d1), Proj(c).proj(1,d2), 'ko', 'markerFaceColor', [0.7 0.9 0.9]);
    
        plot1 = plot(Proj(c).proj(:,d1), Proj(c).proj(:,d2), 'k', 'linewidth',2);
        set(plot1,'color',colors(c,:))
