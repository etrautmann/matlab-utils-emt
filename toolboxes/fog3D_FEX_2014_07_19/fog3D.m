...
function FogProps = CreateFogPlanes(ah,FogProps)
        ...
        YL_             = ylim(ah);
        ZL_             = zlim(ah);
        FogPlaneSize    = max( abs( [ XL_ YL_ ZL_] ) ) * 4; % fog planes should be big enough cover whole scene
        axView          = get(ah,'View');
...
switch lower(Command)
    ...
    case 'add'
        if fogAlreadyAdded(ah)
            ...
            ZL                          = zlim(ah);
            FogProps.Visible            = true;
            FogProps.FollowAxesRotation = true;
            ...
            FogProps.PlaneAlpha         = 1-nthroot(0.05,FogProps.NPlanes);
