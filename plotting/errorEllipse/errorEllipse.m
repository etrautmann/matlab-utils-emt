function h=error_ellipse_emt(varargin)
...
if r==3 & c==3
    ...
    Z(:) = scale*(k*XYZ(:,3)+z0);
    C = ones(size(Z));
    %   h4=surf(X,Y,Z,C,'edgecolor',.3*[1 1 1]);
    h4=surf(X,Y,Z,C,'edgecolor','none');
    colormap gray
    lighting gouraud
    alpha(0.4)
    camlight
...
elseif r==2 & c==2
    ...
    if isnumeric(prop.style)
        h1=plot(scale*(x0+k*x),scale*(y0+k*y));
        set(h1,'color',prop.style);
    else
